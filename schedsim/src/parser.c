...
void print_usage(const char *program_name) {
    ...
    printf("  %s --algorithm=<FCFS|SJF|STCF|RR|MLFQ> --processes=\"P1 0 5; P2 2 3\"\n", program_name);
...
int validate_config(const SchedulerConfig *config) {
    if (config->input_file[0] == '\0' && config->process_input[0] == '\0') {
        fprintf(stderr, "Error: either --input=<file> or --processes=\"...\" is required.\n");
        return 0;
    }
